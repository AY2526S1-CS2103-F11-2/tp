...
Step 4. The user then decides to execute the command `viewall`. Commands that do not modify the address book, such as
`viewall`, will usually not call `Model#commitAddressBook()`, `Model#undoAddressBook()` or `Model#redoAddressBook()`. 
